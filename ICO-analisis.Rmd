...
output: 
  html_document:
    ...
    toc_float:           # Tabla de contenidos flotante a la izquierda
      collapsed: false
      smooth_scroll: true
    code_folding: hide   # Oculta el código por defecto, pero permite verlo con un botón
...
Para este análisis, utilizamos un conjunto de librerías enfocadas en la manipulación de datos (`tidyverse`), análisis estadístico (`rstatix`, `nortest`) y visualización avanzada (`ggplot2`, `flextable`).
...
tryCatch({
  ...
  stop("El archivo 'respuestasEncuesta.xlsx' no se encuentra. Verifica la ruta.")
})
...
datos$f_Motivo <- factor(datos$Motivo,
  levels = c('Conveniencia (falta de tiempo, por trabajo, estudios, etc.)', 'Reuniones sociales (familia, amigos, compañeros de trabajo, etc.)', 'Placer o disfrute personal (por gusto, por probar cosas nuevas, por cambiar de ambiente)', 'Circunstancias extraordinarias (viajes, eventos...)', 'Nunca como fuera de casa'),
  labels = c('Conveniencia', 'Social', 'Placer', 'Extraordinario', 'Nunca'))

datos$f_Lugar_Habitual <- factor(datos$Lugar_Habitual,
  levels = c('En un bar o restaurante', 'En una oficina o centro educativo (cantina, comedor, aula...)', 'En la vía pública (parques, calle, transporte, etc.)'),
...
datos$f_Importancia_Saludable <- factor(datos$Importancia_Saludable, levels = c('1', '2', '3', '4', '5'))

datos$f_Imp_Sostenibilidad <- factor(datos$Imp_Sostenibilidad, levels = c('1', '2', '3', '4', '5'))
datos$f_Imp_Origen_Local <- factor(datos$Imp_Origen_Local, levels = c('1', '2', '3', '4', '5'))
datos$f_Imp_Variedad <- factor(datos$Imp_Variedad, levels = c('1', '2', '3', '4', '5'))
datos$f_Imp_Sabor <- factor(datos$Imp_Sabor, levels = c('1', '2', '3', '4', '5'))
datos$f_Imp_Precio <- factor(datos$Imp_Precio, levels = c('1', '2', '3', '4', '5'))
datos$f_Imp_Higiene <- factor(datos$Imp_Higiene, levels = c('1', '2', '3', '4', '5'))
datos$f_Imp_Rapidez <- factor(datos$Imp_Rapidez, levels = c('1', '2', '3', '4', '5'))
...
datos$f_Aptitud_Social <- factor(datos$Aptitud_Social, levels = c('1', '2', '3', '4', '5'))
...
datos$f_Tipo_Comida <- as.factor(datos$Tipo_Comida)
datos$f_Supermercado_Habitual <- as.factor(datos$Supermercado_Habitual)
datos$f_Situaciones_Uso <- as.factor(datos$Situaciones_Uso)
datos$f_Preferencia_Base <- as.factor(datos$Preferencia_Base)
datos$f_Preferencia_Proteina <- as.factor(datos$Preferencia_Proteina)
datos$f_Preferencia_Complementos <- as.factor(datos$Preferencia_Complementos)
```

## Vista Previa de Datos

A continuación se muestra una vista interactiva de los primeros registros de la base de datos procesada.

```{r vista_previa}
# Usamos DT para una tabla interactiva y scrolleable
DT::datatable(head(datos, 100), 
              options = list(scrollX = TRUE, pageLength = 5),
              caption = "Tabla 1: Vista preliminar de las respuestas")
```
...
```{r demografia}
# Ejemplo de gráfico rápido y estético
p1 <- ggplot(datos, aes(x = Edad, fill = Genero)) +
  geom_bar(position = "dodge", alpha = 0.8) +
  labs(title = "Distribución de Encuestados por Edad y Género",
       y = "Conteo", x = "Rango de Edad") +
...
> **Nota:** Aquí analizamos qué factores son determinantes para el cliente (Sabor, Precio, Higiene).

```{r importancia_factores}
# Selección de columnas de importancia para un resumen
cols_importancia <- datos %>% 
  select(starts_with("Imp_")) %>%
  pivot_longer(cols = everything(), names_to = "Factor", values_to = "Valor")

# (Aquí podrías agregar tu código de Likert o gráficos de barras)
# Ejemplo visual placeholder:
ggplot(cols_importancia, aes(x = reorder(Factor, Valor), y = Valor)) +
  geom_boxplot(fill = "skyblue", outlier.color = "red") +
  coord_flip() +
  labs(title = "Importancia de Atributos en la Elección",
       x = "", y = "Escala de Valoración")
```
...
* **Perfil del usuario:** [Texto autogenerado o escrito a mano sobre quién responde].
* **Oportunidad de mercado:** Basado en la variable `Intencion_Uso`.

---
<br>
<center><small>Informe generado con RMarkdown - `r Sys.Date()`</small></center>   