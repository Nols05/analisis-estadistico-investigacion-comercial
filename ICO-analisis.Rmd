...
proc_freq(datos, 'f_Pago_Agrupado', 'f_Formato.Preferido',
  ...
  include.column_percent = FALSE,
  include.table_percent = FALSE) %>%
  compose(i=2, j=1, value = as_paragraph('Disposición a Pagar'), part = 'header') %>%
  compose(i=1, j=3, value = as_paragraph('Formato Preferido'), part = 'header') %>%
  set_caption('Tabla 36. Contingencia: Disposición a Pagar vs Formato Preferido.') %>%
  autofit()
...
res_chisq_precio %>%
  flextable() %>%
  set_caption('Resultado Chi-cuadrado (Precio vs Formato).') %>%
  colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
  autofit()
...
rstatix::chisq_descriptives(res_chisq_precio) %>%
  ...
  flextable() %>%
  set_caption("Validación de valores esperados.") %>%
  colformat_num(j = c("expected", "std.resid"), big.mark = ".", decimal.mark = ",") %>%
  autofit()
```
...
Por ello, vamos a considerar el test como válido.
...
for (i in seq_along(vars_sociodemo)) {
  ...
  # Preparación de datos específica para cada variable
  datos_temp <- datos
  
  # Si es Género, filtramos para quedarnos solo con Hombre/Mujer (evitamos errores de categorías vacías)
  ...
  t1 <- proc_freq(datos_temp, var_actual, 'f_Intencion.Uso',
            ...
            include.column_percent = FALSE,
            include.table_percent = FALSE) %>%
    compose(i=2, j=1, value = as_paragraph(nombre_actual), part = 'header') %>%
    compose(i=1, j=3, value = as_paragraph('Intención de Uso'), part = 'header') %>%
    set_caption(paste0("Tabla 37.", i, " Contingencia: ", nombre_actual, " vs Aceptación.")) %>%
    autofit()
  print(t1)
  ...
  t2 <- res_chisq %>%
    flextable() %>%
    set_caption(paste0("Resultado Chi-cuadrado (", nombre_actual, ").")) %>%
    colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
    # Resaltamos si es significativo
    bold(i = ~ p < 0.05, bold = TRUE) %>%
    autofit()
  print(t2)
  ...
  t3 <- rstatix::chisq_descriptives(res_chisq) %>%
    ...
    flextable() %>%
    set_caption(paste0("Validación de esperados (", nombre_actual, ").")) %>%
    colformat_num(j = c("expected", "std.resid"), big.mark = ".", decimal.mark = ",") %>%
    autofit()
  print(t3)
