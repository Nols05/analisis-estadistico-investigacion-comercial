...
flextable::set_caption(caption = "Tabla 1. Distribución de frecuencias para Motivo de Comer Fuera.") %>%
  ...
  flextable::align(j = 1, align = "left", part = "body") %>%     # Columna 1 (Motivo) a la izquierda
...
#### `f_Tipo.Comida.Rapida`
Las categorías de `f_Tipo.Comida.Rapida` son si los encuestados comen fuera de casa, si eligen comida rápida o no. Se trata de una variable cuyas categorías no guardan una relación de orden entre sí. Por ese motivo no se definieron los `levels` al factorizarla.

```{r P2_Tipo_Comida_Rapida, fig.height=5}
# 1. PREPARACIÓN DE DATOS
# Contamos la frecuencia (n) y calculamos el porcentaje
df_rapida <- datos %>%
  dplyr::count(f_Tipo.Comida.Rapida, name = "Frecuencia") %>%
  dplyr::mutate(
    Porcentaje = Frecuencia / sum(Frecuencia) * 100
  )

# 2. GENERACIÓN DEL GRÁFICO DE BARRAS (ggplot2)
ggplot(df_rapida, aes(x = f_Tipo.Comida.Rapida, y = Frecuencia, fill = f_Tipo.Comida.Rapida)) +
  # Barras
  ...
  # Se añade una expansión del 10% en el límite superior del eje Y
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) + 
  
  # 4. Títulos y Etiquetas
  labs(
    title = "Figura 3. Consumo de Comida Rápida",
    x = "Consume Comida Rápida",
    y = "Frecuencia (n)",
    fill = "Consume" # Título de la leyenda
  ) +
  # 5. Personalización de color
