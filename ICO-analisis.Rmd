...
Se observa que la base más que se usaría seria la mezcla verde de rúcula, canónigos, espinacas... (con 38 respuestas afirmativas), seguido de pasta (con 29 respuestas afirmativas).
...
```
Se observa que la mayoría de los encuestados salen a comer fuera de casa entre 1 y 2 veces por semana (con un 34.9%). También se observa que el gráfico tiene una ligera asimetría a la derecha.
...
Según la valoración de los factores de importancia en una escala Likert, existe una clara jerarquía de prioridades: los aspectos más cruciales son la Higiene, el Sabor y el Precio, ya que la inmensa mayoría de los encuestados los considera "muy importantes" (categorías 4 y 5); les siguen en importancia la Variedad de Ingredientes, el Origen Local y el valor de Comer Saludable. Por otro lado, la Sostenibilidad es el factor que recibe la mayor proporción de valoraciones bajas en comparación con los demás, y la Rapidez también muestra una importancia variable. Finalmente, una conclusión de opinión destacada es que la mayoría de los encuestados no percibe las ensaladas como una comida social.
...
flextable(
  ...
  autofit()
```
Hemos optado por agrupar las respuestas por categorías para poder analizarlas en mayor profundidad. Nos hemos decantado por 7 categorías:
1. No han tenido malas experiencias.
2. Relacionadas con la higiene, salud y contaminación.
3. Relacionadas con la frescura y calidad.
4. Relacionadas con el sabor y la preparación.
5. Relacionadas con el servicio y la logística detrás de este.
6. Relacionadas con la variedad de ingredientes y la cantidad de porción.
7. Otros (no queda claro donde categorizarlas).

```{r p9_Categorizacion_Cualitativa}
# Simulamos la columna de datos (asumimos que esta columna no se cargó como factor)
respuestas_raw <- c(
  "No he tenido experiencias previas", "No he tenido ninguna mala experiencia que recuerde. No suelo usarlo.", "comida grasienta", "Alguna vez no tenía muy buena pinta, posiblemente había algo en mal estado", "Falta de higiene", "No", "Ninguna", "No", "No", "no", 
  "Me ha aparecido un trozo de madera con pintura en la comida", "Se me olvidaron coger cubiertos y luego no tenía como comer, así que me tocó comer de malas maneras.", "Ocasionalmente el menú de hamburguesas del trinquet, tanto las patatas como la hamburguesa saben a cartón, como si estuviese comiendo pared de casa americana, pero como en relación abusiva, tarde o temprano vuelvo porque está vez cambiaran. También nunca hay pan grande, lo cual es una blasfemia contra la moral pública.", "no", "Comida mal preparada o congelada.", "No he tenido ninguna mala experiencia.", "No", "Un gusano", "Si, que me cuesta mucho decidir que coger", "No", 
  "Algun ingrediente de hojas verdes sin cocinar poco fresco o algo deteriorado por ser un vegetal delicado para conservar", "Falta de frescura", "Control sanitario / Reposición / Dosificación / bacterizacion por contaminacion de clientela, mendigos, viejos, niños", "Los ingredientes no parecían estar muy frescos, o había muy poca variedad de ingredientes.", "Qué esté la comida fría o por lo menos se note que lleva mucho tiempo hecha.", "No", "no", "si", "Buffet de hoteles con poca variedad. La calidad de la comida baja.", "Productos en mal estado", 
  "No he tenido, pero prefiero hacerme las ensaladas en cada", "Es cara y no sabe a nada en específico, pura quimica", "Una vez me intoxique con la comida preparada de Mercadona. A partir de ese momento me lo pienso bastante para volver a comprar los platos preaprados", "No recuerdo", "Ninguna", "En las barras de ensalada ponen mucha lechuga y poco condumio", "No he tenido", "Nunca he tenido malas experiencias", "La verdad es que mientras los productos estén frescos, no.", "No", 
  "No he tenido", "No", "No","No", "Ninguna", "Me preocupa la falta de higiene en la manipulación de los alimentos", "Princi son saludables, no puedo cocinar o calentar los platos preparados", "No", "No he tenido ninguna mala experiencia", "No he tenido ninguna mala experiencia", "no podría describir ninguna. ya que los bares, cafeterías y supermercados en los que suelo comer fuera de casa no tienen implementado esta función de autoservicio.", "No he tenido ninguna", "Falta de higiene", "Problemas con la higiene", "El Sibo", 
  "No comer en Pompeya", "No he tenido", "Comida con mala pinta", "no", "no he tenido", "no", "No he tenido.", "No"
)
# Creamos un dataframe temporal para la categorización
df_cualitativo <- tibble::tibble(respuesta_raw = respuestas_raw)

# Categorización usando case_when (patrones de texto)
df_categorizado <- df_cualitativo %>%
  dplyr::mutate(
    Categoria = dplyr::case_when(
      # Higiene, Salud y Contaminación (Máxima Prioridad)
      stringr::str_detect(respuesta_raw, stringr::regex("gusano|intoxique|madera|higiene|sanitario|contaminacion|Sibo|saludables", ignore_case = TRUE)) ~ "Higiene / Contaminación / Salud",
      
      # Frescura y Calidad
      stringr::str_detect(respuesta_raw, stringr::regex("frescura|fresco|deteriorado|mal estado|mala pinta|fría|baja", ignore_case = TRUE)) ~ "Calidad / Frescura",
      
      # Sabor y Preparación
      stringr::str_detect(respuesta_raw, stringr::regex("grasienta|mal preparada|congelada|cartón|química|saben|caras", ignore_case = TRUE)) ~ "Sabor / Preparación",
      
      # Variedad y Porción
      stringr::str_detect(respuesta_raw, stringr::regex("poca variedad|mucha lechuga|poco condumio|buffet de hoteles", ignore_case = TRUE)) ~ "Variedad / Porción",
      
      # Logística / Servicio
      stringr::str_detect(respuesta_raw, stringr::regex("cubiertos|decidir|calentar|problema|lento", ignore_case = TRUE)) ~ "Logística / Servicio",
      
      # No Mala Experiencia
      stringr::str_detect(respuesta_raw, stringr::regex("no|ninguna|nunca|recuerdo|previas|prefiero|suelo comer fuera|Pompeya", ignore_case = TRUE)) ~ "No Mala Experiencia",
...
df_resumen <- df_categorizado %>%
  dplyr::count(Categoria, name = "Frecuencia") %>%
  dplyr::mutate(
    Porcentaje = (Frecuencia / sum(Frecuencia)) * 100
  ) %>%
  dplyr::arrange(desc(Frecuencia)) # Ordenar por frecuencia


# Añadir Fila Total y Formato (como en la guía)
df_total <- data.frame(
  Categoria = "TOTAL",
...
# Generación de la tabla (flextable)
ft_resumen <- flextable::flextable(df_resumen_final) %>%
  flextable::set_caption(caption = "Tabla 21. Agrupación de Motivos de Mala Experiencia (Respuestas Abiertas)") %>%
  flextable::set_header_labels(
    Categoria = "Categoría de Fallo",
    Frecuencia = "Frecuencia (n)",
    Porcentaje = "Porcentaje (%)"
  ) %>%
  ...
  flextable::hline(i = nrow(df_resumen_final) - 1, border = officer::fp_border(width = 1.5, color = "black")) %>%
  flextable::align(align = "center", part = "all") %>%
  flextable::align(j = 1, align = "left", part = "body") %>%
  flextable::autofit()
...
Procedemos al análisis bivariante-----------------------------------------


Vamos a comprobar si la frecuencia con el que el encuestado come fuera de casa depende del motivo por el que lo hace. Primero, inspeccionaremos la distribución de los casos mediante una tabla de contingencia:
...
'f_Frecuencia',
 ...
 compose(i=2, j=1, value = as_paragraph('Motivo'), part =
'header') %>%
 compose(i=1, j=3, value = as_paragraph('Frecuencia'), part =
'header') %>%
 # indicamos el título
 set_caption('Tabla 8. Tabla de contingencia entre el motivo y
la frecuencia.') %>%
 # ajustamos la tabla a la página
 autofit()

```
Se observa que el 50% de los encuestados que comen fuera de casa por un motivo de conveniencia (trabajo, estudios, etc), lo hacen entre 3 y 4 veces por semana. Además,los encuestados cuyas comidas extradomésticas suelen ser por placer suelen comer fuera de casa ocasionalmente o entre 1 y 2 veces por semana. Finalmente, el 50% de aquellos cuya razón habitual son razones extraordinarias rara vez comen fuera de casa.
...
res.chisq %>%
 ...
 flextable() %>%
 # indicamos el título
 set_caption('Resultados del test del chi cuadrado entre el
...
chisq_descriptives(res.chisq) %>%
 ...
 flextable() %>%
 # indicamos el título
 set_caption('Estadísticos del test del chi cuadrado entre el
...
Al consultar la tabla con los estadísticos descriptivos del test observamos que en la columna "expected" hay valores que son menores de 5. Por tanto el test no puede considerarse válido al no cumplirse una de las condiciones de validez de esta prueba.
Para solucionar este problema, agrupamos las categorías para asegurar que se cumpla esta condición. En este caso, unificamos las categorías "Nunca/Rara" y "Ocasionalmente", así como "3-4/sem" y "5+/sem".
...
'header') %>%
 # indicamos el título
 set_caption('Tabla de contingencia entre el motivo y la frecuencia') %>% # Título ajustado
 # ajustamos la tabla a la página
 autofit()
...
"std.resid"),
 ...
 autofit()
```
Sigue habiendo valores en la columna "expected" menores de 5. Vamos a agrupar "Nunca/Ocasional" con "1-2/sem".
...
proc_freq(datos, 'f_Motivo', 'f_Frecuencia3',
  ...
  set_caption('Tabla de contingencia: Motivo vs Frecuencia (0-2/sem vs 3+/sem)') %>%
  autofit()
```
...
"std.resid"),
 ...
 autofit()
```
La nueva tabla cumple la condición para validar el test del chi cuadrado (valores expected mayores o igual a 5).
Además, el p-valor es menor que 0,05 y por tanto podemos rechazar la hiótesis nula y afirmar que existe una dependencia entre el motivo por el que los encuestados comen fuera y la frecuencia con la que lo hacen. El  54.2% de aquellos cuyo motivo es la conveniencia suelen comer fuera 3 o más veces por semana, mientras que los que comen por placer o por eventos extraordinarios, suelen hacerlo de forma más esporádica, entre 0 y 2 veces por semana.
