...
#### `f_Motivo`
Las categorías de `f_Motivo` son los motivos por los que los encuestados comen fuera de casa. Se trata de una variable cuyas categorías no guardan una relación de orden entre sí. Por ese motivo no se definieron los `levels` al factorizarla.
...
#### `f_Lugar.Habitual`
Las categorías de `f_Lugar.Habitual` son los lugares en los que los encuestados comen fuera de casa. Se trata de una variable cuyas categorías no guardan una relación de orden entre sí. Por ese motivo no se definieron los `levels` al factorizarla.
...
#### `f_Tipo.Comida`
Las categorías de `f_Tipo.Comida` hacen referencia al tipo de comida que los encuestados suelen comer cuando comen fuera de casa. Es importante destacar que esta variable proviene de una pregunta con respuesta multiple por lo que la suma de los porcentajes superará el 100%.

```{r P4-barras, fig.height=5}
# Definir las variables a analizar del grupo "Tipo de Comida"
multi_vars <- c("f_Tipo.Comida.Rapida", "f_Tipo.Comida.Restaurante", "f_Tipo.Comida.Super", 
                "f_Tipo.Comida.Propia")

# Calcular Frecuencia (n) y Porcentaje (%) de 'Sí'
df_multi_res <- datos %>%
  # Seleccionar solo las variables del grupo P4
  dplyr::select(all_of(multi_vars)) %>%
  # Sumar el número de 'Sí' para cada columna
  dplyr::summarise(across(everything(), ~sum(. == 'Sí'))) %>%
  # Transformar de formato ancho a largo (una fila por opción)
  tidyr::pivot_longer(everything(), 
                      names_to = "Tipo_de_Comida", 
                      values_to = "Frecuencia") %>%
  # Calcular el porcentaje y limpiar el texto de las etiquetas
  dplyr::mutate(
    Porcentaje = (Frecuencia / n.filas) * 100,
    # Usamos fixed = TRUE para tratar el patrón como texto literal.
  ...
  # Excluir la opción que nadie marcó (si existiera)
  dplyr::filter(Frecuencia > 0)


# Generación del Gráfico de Barras (horizontal)
ggplot(df_multi_res, aes(x = Porcentaje, y = reorder(Tipo_de_Comida, Porcentaje))) +
  # Barras horizontales
  geom_bar(stat = "identity", fill = "#3182bd") +
  # Etiquetas de Porcentaje sobre las barras
  geom_text(aes(label = paste0(round(Porcentaje, 1), "%")), 
            hjust = -0.1,  # Ajuste horizontal para que quede a la derecha de la barra
            size = 4) + 
  
  # Corrección del eje X para dejar espacio al texto y evitar cortes
  scale_x_continuous(expand = expansion(mult = c(0, 0.2))) + 
  
  # Títulos y Etiquetas
  labs(
    title = "Figura 3. Tipos de Comida Consumida Habitualmente",
    x = "Porcentaje de Encuestados",
    y = NULL # No necesitamos título en el eje Y
  )
```
Se observa una clara dominancia del consumo de comida de restaurante (con un 77,1%), seguido de comida de elaboración propia (con un 31,4%). Estos resultados sugieren que los consumidores están acostumbrados a un formato de comida que prioriza una comida preparada y de calidad.

```{r P4-tabla, fig.height=5}
# Generación de la tabla (flextable)
ft_multi <- flextable::flextable(df_multi_res) %>%
  
  # Añadir Título
  flextable::set_caption(caption = "Tabla 3. Distribución de respuestas para Tipo de Comida (Respuestas Múltiples)") %>%
  
  # Renombrar cabeceras
  flextable::set_header_labels(
    Opcion = "Tipo de Comida",
    Frecuencia = "Frecuencia (n)",
    Porcentaje = "Porcentaje (%)"
  ) %>%
  
  # Formato de datos
  # Aplicamos formato de doble, asumiendo que el porcentaje es numérico (sin el % aún)
  flextable::colformat_double(j = 3, digits = 1, suffix = "%") %>% 
  
  # Aplicar tema y limpiar bordes (se elimina la redundancia)
  flextable::border_remove() %>% 
  flextable::theme_booktabs() %>% 
  
  # Alineación y Cabecera
