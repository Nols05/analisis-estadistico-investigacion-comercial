...
```{r P1}
# Paso 1.1: Crear df_freq usando 'freq' y nombrarlo correctamente
# Asumo que la función 'freq' proviene de la librería 'questionr' o similar.
# El resultado de 'freq' debe ser transformado a data frame y luego manipulado.

df_freq <- as.data.frame(freq(datos$f_Motivo, plot=FALSE)) 

# Paso 1.2: Renombrar las columnas para que coincidan con tu código posterior

# La función 'freq' (si es de 'questionr') típicamente produce columnas
# con nombres como 'modalities' y 'percent'. Debes renombrarlas.
colnames(df_freq) <- c("Motivo", "Frecuencia", "Porcentaje", "Porcentaje_Acumulado") 

# Paso 1.3: Limpiar el data frame para quitar filas que no sirven
# (Como los totales o filas con NA que freq añade a veces)

df_freq <- na.omit(df_freq)
df_freq <- df_freq[df_freq$Motivo != "Total", ] # Eliminar la fila 'Total' si existe

# ¡Importante! Si la función 'freq' incluye una fila de Total, y esa fila
# está nombrada como "Total" en la columna 'Motivo', ¡esta línea funcionará!

# 1. Filtramos los datos para quitar el TOTAL y los que valen 0
# Utilizamos el data frame correctamente nombrado: df_freq
datos_grafico <- df_freq[df_freq$Motivo != "Total" & df_freq$Porcentaje > 0, ]

# Tu código de filtrado (usando la variable ya corregida como df_freq):
# datos_grafico <- df_freq[df_freq$Motivo != "Total" & df_freq$Porcentaje > 0, ]

# 2. Hacemos el gráfico con los datos limpios y etiquetas claras
pie(datos_grafico$Porcentaje,
    # Creamos las etiquetas: Nombre del Motivo + Porcentaje redondeado + '%'
    labels=paste(datos_grafico$Motivo, round(datos_grafico$Porcentaje, 1), "%"),
    # Titulo
