...
pie(datos_grafico$Porcentaje,
    ...
    # Título
    main = 'Figura 9. Motivos de compra.',
...
flextable::set_caption(caption = "Tabla 5. Distribución de frecuencias para Motivo de Comer Fuera.") %>%
...
flextable::set_caption(caption = "Tabla 7. Distribución de frecuencias para Lugar habitual en el que se come fuera de casa.") %>%
...
ggplot(df_barras_dobles, aes(x = Tipo_de_Comida, y = Frecuencia, fill = Respuesta)) +
  ...
  # Títulos y Ejes
  labs(
    title = "Figura 12. Gráfico de barras (Sí/No) por Tipo de Comida",
...
ft_si_no <- flextable::flextable(df_tabla_final) %>%
  ...
  # Añadir Título
  flextable::set_caption(caption = "Tabla 8. Distribución de Frecuencias (Sí/No) para Tipos de Comida") %>%
...
pie(datos_grafico$Porcentaje,
    ...
    labels = datos_grafico$Etiquetas,
    # Título
    main = 'Figura 14. Experiencia con barra de ensaldas.',
    # Asignar un color diferente a cada porción
    col = RColorBrewer::brewer.pal(n = nrow(datos_grafico), name = "Set3"))
...
flextable::set_caption(caption = "Tabla 10. Distribución de frecuencias para Experiencia previa con una barra de ensaladas") %>%
...
ggplot(df_barras_dobles, aes(x = Situaciones_Uso, y = Frecuencia, fill = Respuesta)) +
  ...
  # Títulos y Ejes
  labs(
    title = "Figura 15. Gráfico de barras (Sí/No) por Situaciones de Uso",
    x = "Situación de Uso",
    y = "Número de Respuestas",
    fill = "Usaría"
  ) +
...
ft_si_no <- flextable::flextable(df_tabla_final) %>%
  ...
  # Añadir Título
  flextable::set_caption(caption = "Tabla 11. Distribución de Frecuencias (Sí/No) para Situaciones de Uso") %>%
...
pie(datos_grafico$Porcentaje,
    ...
    labels = datos_grafico$Etiquetas,
    # Título
    main = 'Figura 16. Formato preferido',
    # Asignar un color diferente a cada porción
    col = RColorBrewer::brewer.pal(n = nrow(datos_grafico), name = "Set3"))
...
flextable::set_caption(caption = "Tabla 12. Distribución de frecuencias para Formato preferido") %>%
...
ggplot(df_barras_dobles, aes(x = Preferencia_Base, y = Frecuencia, fill = Respuesta)) +
  ...
  # Títulos y Ejes
  labs(
    title = "Figura 17. Gráfico de barras (Sí/No) por Preferencia de Base",
    x = "Preferencia Base",
    y = "Número de Respuestas",
    fill = "Usaría"
  ) +
...
ft_si_no <- flextable::flextable(df_tabla_final) %>%
  ...
  # Añadir Título
  flextable::set_caption(caption = "Tabla 13. Distribución de Frecuencias (Sí/No) para Preferencia Base") %>%
...
ggplot(df_barras_dobles, aes(x = Prerencia_Proteina, y = Frecuencia, fill = Respuesta)) +
  ...
  # Títulos y Ejes
  labs(
    title = "Figura 18. Gráfico de barras (Sí/No) por Preferencia de Proteínas",
...
ft_si_no <- flextable::flextable(df_tabla_final) %>%
  ...
  # Añadir Título
  flextable::set_caption(caption = "Tabla 14. Distribución de Frecuencias (Sí/No) para Preferencia de Proteína") %>%
...
ggplot(df_barras_dobles, aes(x = Preferenica_Complementos, y = Frecuencia, fill = Respuesta)) +
  ...
  # Títulos y Ejes
  labs(
    title = "Figura 19. Gráfico de barras (Sí/No) por Preferencia Complementos",
...
ft_si_no <- flextable::flextable(df_tabla_final) %>%
  ...
  # Añadir Título
  flextable::set_caption(caption = "Tabla 15. Distribución de Frecuencias (Sí/No) para Preferencia de Complementos") %>%
...
pie(datos_grafico$Porcentaje,
    ...
    labels = datos_grafico$Etiquetas,
    # Título
    main = 'Figura 21. Preferencia del Formato de Dispensación',
    # Asignar un color diferente a cada porción
    col = RColorBrewer::brewer.pal(n = nrow(datos_grafico), name = "Set3"))
...
flextable::set_caption(caption = "Tabla 17. Distribución de frecuencias para Formato de Dispensación Preferido") %>%
...
pie(datos_grafico$Porcentaje,
    ...
    labels = datos_grafico$Etiquetas,
    # Título
    main = 'Figura 22. Grado de Aceptación',
    # Asignar un color diferente a cada porción
    col = RColorBrewer::brewer.pal(n = nrow(datos_grafico), name = "Set3"))
...
flextable::set_caption(caption = "Tabla 18. Distribución de frecuencias para el Grado de Aceptación") %>%
...
flextable::set_caption(caption = "Tabla 2. Distribución de frecuencias para Género") %>%
...
pie(datos_grafico$Porcentaje,
    ...
    labels = datos_grafico$Etiquetas,
    # Título
    main = 'Figura 8. Situación Laboral',
    # Asignar un color diferente a cada porción
    col = RColorBrewer::brewer.pal(n = nrow(datos_grafico), name = "Set3"))
...
flextable::set_caption(caption = "Tabla 4. Distribución de frecuencias para Situación Laboral") %>%
...
ggplot(df_frecuencia_tiempo, aes(x = f_Frecuencia, y = Frecuencia)) +
  ...
  # Títulos y Ejes
  labs(
    title = 'Figura 10. Frecuencia de Comer Fuera de Casa',
    x = "Frecuencia",
...
flextable::set_caption(caption = "Tabla 6. Distribución de frecuencias para Frecuencia de Comer Fuera de Casa") %>%
...
ggplot(df_frecuencia_tiempo, aes(x = f_Frecuencia.Comida.Preparada, y = Frecuencia)) +
  ...
  # Títulos y Ejes
  labs(
    title = 'Figura 13. Frecuencia de Comprar Comida Preparada en Supermercados',
...
flextable::set_caption(caption = "Tabla 9. Distribución de frecuencias para Frecuencia de Comprar Comida Preparada en un Supermercado") %>%
...
####`f_Disposicion.Pago` 
Las categorías de `f_Disposicion.Pago` miden la disposición a pagar de los encuestados por las ensaladas de la barra.
...
ggplot(df_frecuencia_tiempo, aes(x = f_Disposicion.Pago, y = Frecuencia)) +
  ...
  # Títulos y Ejes
  labs(
    title = 'Figura 20. Disposición a Pagar',
...
flextable::set_caption(caption = "Tabla 16. Distribución de frecuencias para la Disposición a Pagar") %>%
...
flextable::set_caption(caption = "Tabla 1. Distribución de frecuencias por Edad") %>%
...
ggplot(df_frecuencia_tiempo, aes(x = f_Nivel.Educativo, y = Frecuencia)) +
  ...
  # Títulos y Ejes
  labs(
    title = 'Figura 7. Nivel Educativo Máximo Alcanzado',
    x = "Nivel Educativo",
    y = "Numero de respuestas"
  ) +
...
flextable::set_caption(caption = "Tabla 3. Distribución de frecuencias para Nivel Educativo Máximo Alcanzado") %>%
...
# 3.5 RE-ETIQUETADO DE FACTORES
new_likert_labels <- c(
  'Nada Importante / Nada apropiado', 
  'Ligeramente Importante / Poco apropiado',          
  'Moderado / Neutral',              
  'Importante / Apropiado',          
  'Muy Importante / Muy apropiado'   
)

# Aplicamos las nuevas etiquetas a todas las columnas Likert seleccionadas
datos.likert[] <- lapply(datos.likert, function(x) {
  levels(x) <- new_likert_labels
  return(x)
})
...
likert::likert(datos.likert), 
 ...
 # Eliminamos la etiqueta genérica de la leyenda, ya que los niveles son autoexplicativos
 legend.position = 'bottom'
) +
 # Título de la figura (Actualizado)
 labs(
    # ESTA ES LA CLAVE: Usamos 'fill' para renombrar el título de la leyenda
fill = "Nivel de Valoración", 
title = 'Figura X. Valoración de Factores de Importancia y Aptitud Social',
 y = 'Porcentaje (%)',
 x = NULL
)
...
```{r likert-descriptivos}
# 1. Variables y Etiquetas (Se asume que los datos están cargados y filtrados)
likert_vars <- c(
  'Importancia.Saludable', 'Imp.Sostenibilidad', 'Imp.Origen.Local', 
  'Imp.Variedad', 'Imp.Sabor', 'Imp.Precio', 'Imp.Higiene', 
  'Imp.Rapidez', 'Aptitud.Social'
)

likert_labels <- c(
  'Importancia de comer saludable',
  'Importancia de la sostenibilidad',
  'Importancia del origen local',
  'Importancia de la variedad',
  'Importancia del sabor',
  'Importancia del precio',
  'Importancia de la higiene',
  'Importancia de la rapidez',
  'Aptitud social de las ensaladas'
)

# 2. Selección y Conversión a Numérico
# Se asume que 'datos' es el DataFrame filtrado con N=61 (o N=63, según la ejecución)
datos_likert <- datos[, likert_vars]
datos_likert[] <- lapply(datos_likert, as.numeric)

# 3. Función para calcular descriptivos (usa base R)
f_descriptivos <- function(x) {
  data.frame(
    n = sum(!is.na(x)),
...
# 4. Cálculo de Descriptivos
descriptivos_raw <- lapply(datos_likert, f_descriptivos)
descriptivos_final <- do.call(rbind, descriptivos_raw)
descriptivos_final$Variable <- likert_labels
rownames(descriptivos_final) <- NULL

# Reordenar columnas para la tabla
descriptivos_final <- descriptivos_final %>%
  select(Variable, n, min, max, median, mean, sd)

# 5. Generación de la flextable
ft_descriptivos <- flextable(descriptivos_final) %>%
  set_caption(caption = "Tabla 19. Estadísticos Descriptivos para Escalas Likert") %>%
  set_header_labels(
    Variable = "Factor de Decisión",
    n = "N",
    min = "Mín.",
    max = "Máx.",
    median = "Mediana",
  ...
  colformat_double(j = c("min", "max", "median", "n"), digits = 0) %>%
  theme_booktabs() %>%
...
flextable::flextable(df_resumen_final) %>%
  flextable::set_caption(caption = "Tabla 20. Agrupación de Motivos de Mala Experiencia (Datos Reales).") %>%
  flextable::set_header_labels(
...
## H1: Relacionar la frecuencia con la que la muestra come fuera de casa y el motivo principal
...
## H2: Relacionar la frecuencia con la que la muestra compra comida preparada de supermercado y el motivo principal
...
proc_freq(datos, 'f_Motivo', 'f_Frecuencia.Comida.Preparada',
 ...
 include.column_percent = FALSE,
 include.table_percent = FALSE) %>%
 compose(i=1, j=1, value = as_paragraph('Motivo'), part = 'header') %>%
 compose(i=1, j=3, value = as_paragraph('Frecuencia Comida Preparada'), part = 'header') %>%
 set_caption('Tabla 33. Tabla de contingencia entre el motivo y la frecuencia de compra de comida preparada.') %>%
 autofit()
```
...
res.chisq.prep %>%
 flextable() %>%
 set_caption('Tabla 34. Resultados del test del chi cuadrado (Comida Preparada') %>%
 colformat_num(
   j = c("statistic", "p"),
   big.mark = ".",
   decimal.mark = ","
...
chisq_descriptives(res.chisq.prep) %>%
 rename(
   ...
   Frecuencia_Prep = 'y') %>%
 flextable() %>%
 set_caption('Tabla 35. Estadísticos descriptivos del test (Valores esperados y residuos).') %>%
 colformat_num(
...
proc_freq(datos, 'f_Motivo', 'f_Frecuencia.Prep.Agrupada',
  ...
  set_caption('Tabla 36. Tabla de contingencia: Motivo vs Frecuencia Comida Preparada (Agrupada)') %>%
  autofit()
...
res.chisq.prep.agrup %>%
  flextable() %>%
  set_caption('Tabla 37. Resultados finales del test Chi-cuadrado (Agrupado)') %>%
...
chisq_descriptives(res.chisq.prep.agrup) %>%
 ...
 flextable() %>%
 set_caption('Tabla 38. Estadísticos del test Chi-cuadrado (Agrupado) con residuos estandarizados.') %>%
 colformat_num(
   j = c("prop", "row.prop", "col.prop", "expected", "resid", "std.resid"),
...
-   **Ocio y Otros (Perfil Hedónico):** Agrupamos "Social", "Placer" y "Extraordinario", ya que todos comparten que la ingesta es por disfrute o circunstancias voluntarias, no por obligación.
    ...
    proc_freq(datos, 'f_Motivo_Binario', 'f_Frecuencia.Prep.Agrupada',
      ...
      include.column_percent = FALSE,
      include.table_percent = FALSE) %>%
      compose(i=1, j=1, value = as_paragraph('Tipo de Motivo'), part = 'header') %>%
      compose(i=1, j=3, value = as_paragraph('Frecuencia Compra Prep.'), part = 'header') %>%
      set_caption('Tabla 39. Tabla de contingencia: Motivos Funcionales vs Ocio y Frecuencia de Compra.') %>%
      autofit()
    ...
    res.chisq.final %>%
      flextable() %>%
      set_caption('Tabla 40. Resultados del test Chi-cuadrado (Modelo 2x2).') %>%
      colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
      autofit()
    ...
    chisq_descriptives(res.chisq.final) %>%
     ...
     flextable() %>%
     set_caption('Tabla 41. Estadísticos del test (Valores esperados y residuos).') %>%
     colformat_num(
...
## H3: Contrastar la creencia de que se recurre a la comida rápida por falta de tiempo.
...
proc_freq(datos, 'f_Motivo_Binario', 'f_Tipo.Comida.Rapida',
  ...
  set_caption('Tabla 42. Tabla de contingencia: Conveniencia o no y Comida Rápida.') %>%
  autofit()
...
res.chisq.fastfood.final %>%
  flextable() %>%
  set_caption('Tabla 43. Resultados del test Chi-cuadrado (Modelo 2x2).') %>%
  colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
  autofit()
...
chisq_descriptives(res.chisq.fastfood.final) %>%
 ...
 flextable() %>%
 set_caption('Tabla 44. Estadísticos finales y residuos estandarizados.') %>%
 colformat_num(
   j = c("prop", "row.prop", "col.prop", "expected", "resid", "std.resid"),
   big.mark = ".", decimal.mark = ","
 ) %>%
...
## H4: Establecer la relación entre los lugares a los que se acude cuando se come fuera de casa (bar o restaurante, trabajo o centro educativo, vía pública) y el tipo de comida elegido (rápida, platos preparados, de elaboración propia o comida preparada en un restaurante).
...
#### H4.1: Asociación con Comida de Rápida
...
proc_freq(datos, 'f_Lugar.Habitual', 'f_Tipo.Comida.Rapida',
 ...
 include.column_percent = FALSE,
 include.table_percent = FALSE) %>%
 compose(i=1, j=1, value = as_paragraph('Lugar Habitual'), part = 'header') %>%
 compose(i=1, j=3, value = as_paragraph('Consume Comida Rápida'), part = 'header') %>%
 set_caption('Tabla 45. Contingencia: Lugar Habitual vs Comida Rápida.') %>%
 autofit()
...
res.chisq.rapida %>%
 flextable() %>%
 set_caption('Tabla 46. Resultados del test Chi-cuadrado (Comida Rápida).') %>%
 colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
 autofit()
...
chisq_descriptives(res.chisq.rapida) %>%
 ...
 flextable() %>%
 set_caption('Tabla 47. Estadísticos y valores esperados: Lugar vs Comida Rápida.') %>%
 colformat_num(
   j = c("prop", "row.prop", "col.prop", "expected", "resid", "std.resid"),
   big.mark = ".", decimal.mark = ","
 ) %>%
...
proc_freq(datos, 'f_Lugar_Agrupado', 'f_Tipo.Comida.Rapida',
 ...
 compose(i=1, j=1, value = as_paragraph('Lugar (Agrupado)'), part = 'header') %>%
 compose(i=1, j=3, value = as_paragraph('Consume Comida Rápida'), part = 'header') %>%
 set_caption('Tabla 48. Contingencia: Lugar vs Comida Rápida.') %>%
 autofit()

# Test Chi-cuadrado
res.chisq.rapida <- chisq_test(x = datos$f_Lugar_Agrupado, y = datos$f_Tipo.Comida.Rapida)
res.chisq.rapida %>% flextable() %>%
 set_caption('Tabla 49. Test Chi-cuadrado (Comida Rápida).') %>%
 colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>% autofit()

# Validación (Esperados)
chisq_descriptives(res.chisq.rapida) %>%
 rename(Lugar = 'x', Comida_Rapida = 'y') %>% flextable() %>%
 set_caption('Tabla 50. Estadísticos y residuos (Comida Rápida).') %>%
...
#### H4.2: Asociación con Platos Preparados de Supermercado
...
proc_freq(datos, 'f_Lugar_Agrupado', 'f_Tipo.Comida.Super',
 ...
 set_caption('Tabla 51. Contingencia: Lugar vs Platos de Supermercado.') %>%
 autofit()

# Test Chi-cuadrado
res.chisq.super <- chisq_test(x = datos$f_Lugar_Agrupado, y = datos$f_Tipo.Comida.Super)
res.chisq.super %>% flextable() %>%
 set_caption('Tabla 52. Test Chi-cuadrado (Platos Supermercado).') %>%
 colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>% autofit()

# Validación
chisq_descriptives(res.chisq.super) %>%
 rename(Lugar = 'x', Comida_Super = 'y') %>% flextable() %>%
 set_caption('Tabla 53. Estadísticos y residuos (Platos Supermercado).') %>%
 colformat_num(j = c("expected", "std.resid"), big.mark = ".", decimal.mark = ",") %>% autofit()
```

Como hay un valor excepted ligaramente menor que 5 realizamos el test de Fisher:

```{r}
tabla_cruzada <- table (datos$f_Lugar_Agrupado, datos$f_Tipo.Comida.Super)

res.fisher <- fisher_test(tabla_cruzada)

res.fisher %>%
  flextable() %>%
  set_caption(' Tabla 54. Resultados del Test de Fisher') %>%
  colformat_num(
    j = c("p"),
    big.mark = ".",
    decimal.mark = ","
...
#### H4.3: Asociación con Comida de Elaboración Propia (Tupper)
...
#### H4.4: Asociación con Comida de Bar o Restaurante
...
## H6: Relación entre la sensación de saciedad y la aceptación
...
proc_freq(datos, 'f_Perfil_Saciedad', 'f_Intencion.Uso',
  ...
  include.column_percent = FALSE,
  include.table_percent = FALSE) %>%
  compose(i=1, j=1, value = as_paragraph('Percepción de Saciedad'), part = 'header') %>%
  compose(i=1, j=3, value = as_paragraph('Intención de Uso'), part = 'header') %>%
  set_caption('Tabla 61. Tabla de contingencia: Percepción de Saciedad vs Intención de Uso.') %>%
  autofit()
...
res.chisq.saciedad %>%
  flextable() %>%
  set_caption('Tabla 62. Resultados del test Chi-cuadrado (Saciedad).') %>%
  colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
  autofit()
...
chisq_descriptives(res.chisq.saciedad) %>%
 ...
 flextable() %>%
 set_caption('Tabla 63. Estadísticos del test (Valores esperados y residuos).') %>%
 colformat_num(
   j = c("prop", "row.prop", "col.prop", "expected", "resid", "std.resid"),
   big.mark = ".", decimal.mark = ","
 ) %>%
...
## H7: Relacionar la relevancia de comer saludable con la aceptación de la barra de ensaladas

Primero observamos las medias y la distribución de las puntuaciones en cada grupo.

```{r}
# 1. Estadísticos descriptivos por grupo
datos %>%
  group_by(f_Intencion.Uso) %>%
  rstatix::get_summary_stats(Importancia.Saludable, type = "common") %>%
  flextable::flextable() %>%
  compose(i=1, j=1, value = as_paragraph('Aceptación'), part = 'header') %>%
  compose(i=1, j=2, value = as_paragraph('Importancia de comer saludable'), part = 'header') %>%
  set_caption("Tabla 64. Estadísticos descriptivos: Importancia Saludable (1-5) según Intención de Uso.") %>%
  colformat_double(j = c("mean", "sd", "se", "ci"), digits = 2, decimal.mark = ",") %>%
  autofit()

# 2. Diagrama de Caja y Bigotes
ggpubr::ggboxplot(datos, 
          x = "f_Intencion.Uso", 
          y = "Importancia.Saludable",
          color = "f_Intencion.Uso", 
          palette = c("#E7B800", "#2E9FDF"),
          add = "jitter",
          ylab = "Importancia de comer saludable (1-5)",
          xlab = "Aceptación",
          title = "Figura 24. Distribución de la Importancia Saludable según Aceptación")
```
En el gráfico de bigotes se aprecia que existe un solapamiento considerable entre ambas cajas, lo que sugiere que no hay una diferencia radical entre la preocupación por la salud entre ambos grupos. Además, las medianas parecen posicionarse en valores similares (3,5 - 4) y hay una gran dispersión en los datos. Esto anticipa que la preocupación por la salud podría no ser el factor determinante de la aceptación del producto.

Comprobamos si los residuos del modelo siguen una distribución normal para decidir si usamos un test paramétrico (ANOVA) o no paramétrico (Kruskal-Wallis). Usamos el Test de Anderson-Darling ($n > 50$).

```{r}
# 1. Construimos el modelo lineal para extraer residuos
modelo_salud <- lm(Importancia.Saludable ~ f_Intencion.Uso, data = datos)

# 2. Gráfico QQ Plot
ggpubr::ggqqplot(residuals(modelo_salud)) +
  labs(title = "Figura 25. Gráfico QQ de los residuos (Importancia Saludable)")

# 3. Test de Anderson-Darling
test_ad <- nortest::ad.test(residuals(modelo_salud))

# Imprimimos el resultado para confirmar la NO normalidad
data.frame(
  Metodo = test_ad$method,
  Estadistico = test_ad$statistic,
  P_Valor = test_ad$p.value
) %>%
  flextable::flextable() %>%
  set_caption("Tabla 65. Test de Normalidad de Anderson-Darling (Residuos).") %>%
  colformat_double(j = "P_Valor", digits = 5, decimal.mark = ",") %>%
  # Resaltamos en rojo si NO es normal
  color(i = ~ P_Valor < 0.05, color = "red") %>%
  autofit()
```

Dado los datos no siguen una distribución normal (lo cual es esperable en escalas Likert), aplicamos la Prueba de Kruskal-Wallis.

```{r}
# Ejecución directa del test no paramétrico
res_kruskal <- rstatix::kruskal_test(datos, Importancia.Saludable ~ f_Intencion.Uso)

# Impresión de resultados
res_kruskal %>%
  flextable::flextable() %>%
  set_caption("Tabla 66. Resultados del Test de Kruskal-Wallis (Relevancia Salud vs Aceptación).") %>%
  colformat_double(j = c("statistic", "p"), digits = 3, decimal.mark = ",") %>%
  # Negrita si es significativo
  bold(i = ~ p < 0.05, bold = TRUE) %>%
  autofit()
```

El p-valor es mayor que 0,05. Por tanto concluimos que no existe una dependencia entre la importancia que los encuestados dan a comer saludable con la aceptación de la barra de ensaladas.


## H8: Comparar el interés potencial entre quienes comen fuera y quienes no
...
proc_freq(datos, 'f_Comer_Fuera_Binaria', 'f_Intencion.Uso',
  ...
  include.column_percent = FALSE,
  include.table_percent = FALSE) %>%
  compose(i=1, j=1, value = as_paragraph('Hábito de Comer Fuera'), part = 'header') %>%
  compose(i=1, j=3, value = as_paragraph('Intención de Uso'), part = 'header') %>%
  set_caption('Tabla 67. Tabla de contingencia: Hábito de Comer Fuera vs Intención de Uso.') %>%
  autofit()
...
res.chisq.habito %>%
  flextable() %>%
  set_caption('Tabla 68. Resultados del test Chi-cuadrado (Hábito).') %>%
  colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
  autofit()
...
chisq_descriptives(res.chisq.habito) %>%
 ...
 flextable() %>%
  set_caption('Tabla 69. Estadísticos del test (Valores esperados y residuos).') %>%
 colformat_num(
   j = c("prop", "row.prop", "col.prop", "expected", "resid", "std.resid"),
   big.mark = ".", decimal.mark = ","
 ) %>%
...
## H9: Analizar cómo las malas experiencias anteriores con platos preparados o autoservicio influyen en el interés potencial por el producto.
...
proc_freq(datos, 'f_Exp_Negativa_Binaria', 'f_Intencion.Uso',
  ...
  compose(i=1, j=3, value = as_paragraph('Intención de Uso'), part = 'header') %>%
  set_caption('Tabla 70. Tabla de contingencia: Malas Experiencias Previas (Binaria) vs Intención de Uso.') %>%
  autofit()
...
res.chisq.negativa %>%
  flextable() %>%
  set_caption('Tabla 71. Resultados del test Chi-cuadrado (Malas Experiencias).') %>%
  colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
  autofit()
...
chisq_descriptives(res.chisq.negativa) %>%
 ...
 flextable() %>%
 set_caption('Tabla 72. Estadísticos del test (Valores esperados y residuos).') %>%
 colformat_num(
   j = c("prop", "row.prop", "col.prop", "expected", "resid", "std.resid"),
   big.mark = ".", decimal.mark = ","
 ) %>%
...
## H10: Vincular el formato de dispensación preferido con las características consideradas más prioritarias (sostenibilidad, precio, higiene, rapidez del servicio, sabor, variedad y origen de los ingredientes)
...
flextable::flextable(tabla_normalidad) %>%
  set_caption("Tabla 73. Test de Normalidad de Anderson-Darling (Residuos).") %>%
...
res_kruskal %>%
  ...
  flextable::flextable() %>%
  set_caption("Tabla 74. Resultados del Test de Kruskal-Wallis (Dependencia Final).") %>%
  colformat_double(j = c("statistic", "p"), digits = 3, decimal.mark = ",") %>%
...
Para todas las variables analizadas (Higiene, Origen Local, Precio, Rapidez, Sabor, Sostenibilidad y Variedad), se han obtenido p-valores muy superiores al nivel de significación de 0,05 (oscilando entre 0,112 y 0,959).
...
## H11: Relación entre los ingredientes demandados y la edad
...
```{r}
# 1. Identificación de los "Top 3" Ingredientes
...
# 2. Función para realizar el análisis detallado (Numeración corregida)
realizar_analisis_edad <- function(var_ingrediente, nombre_ingrediente, i) {
  
  # Calculamos el desplazamiento: 
  # Para i=1 -> tablas 75, 76, 77
  # Para i=2 -> tablas 78, 79, 80
  # Para i=3 -> tablas 81, 82, 83
  base_num <- 74 + (i - 1) * 3
  ...
  t1 <- proc_freq(datos, var_ingrediente, 'f_Edad',
            ...
            include.column_percent = FALSE,
            include.table_percent = FALSE) %>%
    compose(i=1, j=1, value = as_paragraph(nombre_ingrediente), part = 'header') %>%
    compose(i=1, j=3, value = as_paragraph('Edad'), part = 'header') %>%
    set_caption(paste0("Tabla ", base_num + 1, ". Tabla de contingencia: ", nombre_ingrediente, " vs Edad.")) %>%
    autofit()
  print(t1)
  ...
  t2 <- res_chisq %>%
    flextable() %>%
    set_caption(paste0("Tabla ", base_num + 2, ". Resultados del test Chi-cuadrado (", nombre_ingrediente, ").")) %>%
    colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
    autofit()
  print(t2)
  
  # C. Descriptivos (Valores esperados)
  t3 <- rstatix::chisq_descriptives(res_chisq) %>%
    dplyr::rename(Ingrediente = 'x', Edad = 'y') %>%
    flextable() %>%
    set_caption(paste0("Tabla ", base_num + 3, ". Estadísticos del test (Valores esperados): ", nombre_ingrediente, ".")) %>%
...
# 3. Ejecución del bucle pasando el índice 'i'
for (i in 1:3) {
  realizar_analisis_edad(top_ingredientes[i], nombres_limpios[i], i)
}
```
...
# 3. Bucle de Análisis con numeración de 84 a 92
for (i in 1:3) {
  var_ingrediente <- top_ingredientes[i]
  nombre_ingrediente <- nombres_limpios[i]
  
  # Calculamos la base: para i=1 es 83, para i=2 es 86, para i=3 es 89
  base_num <- 83 + (i - 1) * 3
  ...
  t1 <- proc_freq(datos, var_ingrediente, 'f_Edad_Agrupada',
    ...
    set_caption(paste0("Tabla ", base_num + 1, ". Contingencia: ", nombre_ingrediente, " vs Edad Agrupada.")) %>%
    autofit()
  print(t1)
  ...
  t2 <- res_chisq %>%
    flextable() %>%
    set_caption(paste0("Tabla ", base_num + 2, ". Resultado Chi-cuadrado (", nombre_ingrediente, ").")) %>%
    colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
    autofit()
  print(t2)
...
## H12: Relación entre los ingredientes más demandados y el género
...
# 3. Bucle de Análisis con numeración de 93 a 101
for (i in 1:3) {
  var_ingrediente <- top_ingredientes[i]
  nombre_ingrediente <- nombres_limpios[i]
  
  # Base numérica: 92 + (i-1)*3
  # i=1 -> 92 | i=2 -> 95 | i=3 -> 98
  base_num <- 92 + (i - 1) * 3
  ...
  t1 <- proc_freq(datos_genero, var_ingrediente, 'f_Genero',
    ...
    set_caption(paste0("Tabla ", base_num + 1, ". Contingencia: ", nombre_ingrediente, " vs Género.")) %>%
    autofit()
  print(t1)
...
## H13: Influencia del precio dispuesto a pagar en el modelo de tarificación

Analizamos si existe una relación entre cuánto está dispuesto a pagar el consumidor y qué modelo de precios prefiere (Precio Fijo por tamaño vs. Precio por Peso).

Para evitar problemas de validez estadística por la dispersión de respuestas en los extremos, hemos agrupado la disposición a pagar en dos niveles: **"Hasta 5,50€"** y **"Más de 5,50€"**.

```{r}
# 1. Agrupación de la Disposición a Pagar (Para robustez del test)
datos$f_Pago_Agrupado <- datos$f_Disposicion.Pago
levels(datos$f_Pago_Agrupado) <- list(
  'Hasta 5,50€'   = c('<4€', '4-5.5€'),
  'Más de 5,50€'  = c('5.5-7€', '7-9.5€', '>9.5€')
)

# 2. Tabla de Contingencia
proc_freq(datos, 'f_Pago_Agrupado', 'f_Formato.Preferido',
  include.row_percent = TRUE,
  include.column_percent = FALSE,
  include.table_percent = FALSE) %>%
  compose(i=1, j=1, value = as_paragraph('Disposición a Pagar'), part = 'header') %>%
  compose(i=1, j=3, value = as_paragraph('Formato Preferido'), part = 'header') %>%
  set_caption('Tabla 102. Contingencia: Disposición a Pagar vs Formato Preferido.') %>%
  autofit()

# 3. Test Chi-cuadrado
res_chisq_precio <- rstatix::chisq_test(
  x = datos$f_Pago_Agrupado,
  y = datos$f_Formato.Preferido
)

res_chisq_precio %>%
  flextable() %>%
  set_caption('Tabla 103. Resultado Chi-cuadrado (Precio vs Formato).') %>%
  colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
  autofit()

# 4. Validación (Expected)
rstatix::chisq_descriptives(res_chisq_precio) %>%
  dplyr::rename(Pago = 'x', Formato = 'y') %>%
  flextable() %>%
  set_caption("Tabla 104. Estadísiticos del test (Validación de valores esperados).") %>%
  colformat_num(j = c("expected", "std.resid"), big.mark = ".", decimal.mark = ",") %>%
  autofit()
```

Todos los valores expected son mayores que 5, excepto uno que es un poco mejor que 5.
Esto es debe a falta de muestra, ya que hay pocas respuestas para esa combinación de variables.
Por ello, vamos a hacer el test de Fisher.

omo hay un valor excepted ligaramente menor que 5 realizamos el test de Fisher:

```{r}
tabla_cruzada <- table (datos$f_Pago_Agrupado, datos$f_Formato.Preferido)

res.fisher <- fisher_test(tabla_cruzada)

res.fisher %>%
  flextable() %>%
  set_caption(' Tabla 105. Resultados del Test de Fisher') %>%
  colformat_num(
    j = c("p"),
    big.mark = ".",
    decimal.mark = ","
...
El p-valor es mayor que 0,05, por tanto se rechaza la hipótesis nula y concluimos que no hay una dependencia entre el precio dispuesto a pagar y el formato de tarificación preferido.
Ambos grupos prefieren pagar un precio fijo.

## H14: Analizar el grado de aceptación del producto según las variables sociodemográficas (edad, género, nivel educativo y ocupación).

Analizamos el grado de aceptación del producto en función de las características sociodemográficas de los encuestados: Edad, Género, Nivel Educativo y Situación Laboral.

```{r}
# 1. Definición de variables y etiquetas
vars_sociodemo <- c("f_Edad", "f_Genero", "f_Nivel.Educativo", "f_Situacion.Laboral")
nombres_vars <- c("Edad", "Género", "Nivel Educativo", "Situación Laboral")

# 2. Bucle de Análisis Automático
for (i in seq_along(vars_sociodemo)) {
  
  var_actual <- vars_sociodemo[i]
  nombre_actual <- nombres_vars[i]
  
  # Calculamos el número base para esta iteración
  # i=1 -> 105 | i=2 -> 108 | i=3 -> 111 | i=4 -> 114
  base_num <- 105 + (i - 1) * 3
  
  # Preparación de datos específica
  datos_temp <- datos
  
  if (var_actual == "f_Genero") {
    datos_temp <- datos %>% 
      dplyr::filter(f_Genero %in% c("Hombre", "Mujer")) %>% 
      droplevels()
  }
  
  cat(paste("\n### Análisis de Aceptación según:", nombre_actual, "\n"))
  
  # A. Tabla de Contingencia
  t1 <- proc_freq(datos_temp, var_actual, 'f_Intencion.Uso',
            include.row_percent = TRUE,
            include.column_percent = FALSE,
            include.table_percent = FALSE) %>%
    # Corrección de los compose para los headers
    compose(i=1, j=1, value = as_paragraph(nombre_actual), part = 'header') %>%
    compose(i=1, j=3, value = as_paragraph('Intención de Uso'), part = 'header') %>%
    set_caption(paste0("Tabla ", base_num + 1, ". Contingencia: ", nombre_actual, " vs Aceptación.")) %>%
    autofit()
  print(t1)
  
  # B. Test Chi-Cuadrado
  res_chisq <- rstatix::chisq_test(x = datos_temp[[var_actual]], y = datos_temp$f_Intencion.Uso)
  
  t2 <- res_chisq %>%
    flextable() %>%
    set_caption(paste0("Tabla ", base_num + 2, ". Resultado Chi-cuadrado (", nombre_actual, ").")) %>%
    colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
    bold(i = ~ p < 0.05, bold = TRUE) %>%
    autofit()
  print(t2)
  
  # C. Validación (Valores Esperados)
  t3 <- rstatix::chisq_descriptives(res_chisq) %>%
    dplyr::rename(Variable = 'x', Aceptacion = 'y') %>%
    flextable() %>%
    set_caption(paste0("Tabla ", base_num + 3, ". Estadísticos del test (Valores esperados): ", nombre_actual, ".")) %>%
    colformat_num(j = c("expected", "std.resid"), big.mark = ".", decimal.mark = ",") %>%
    autofit()
  print(t3)
...
De las 4 variables sociodemográficas, solo Género cuenta con valores “expected” > 5, por tanto, primeramente analizaremos esta. Como su p-valor > 0,05 se rechaza la hipótesis nula y concluimos que no hay una dependencia entre el género de los encuestados y el grado de aceptación de la barra de ensaladas. Señalamos que ambos géneros tienen un grado de aceptación alto.
Para continuar con el análisis de las variables restantes agrupamos por diferentes grupos:

- Para **Edad** agruparemos como en análisis anteriores: "Jóvenes" (hasta 25 años) y "No Jóvenes" (+ 25 años) para concentrar en un grupo la mayoría de la muestra.
- Para **Nivel Educativo** agruparemos en 2 categorías: "Nivel de Estudios Medios" (ESO y FP medio / Bachillerato) y "Nivel de Estudios Superiores" (FP superior y Universidad).
- Para **Situación Laboral** agruparemos por 2 categorías "Estudiantes" y "No Estudiantes" (Trabajadores por cuenta ajena, Autónomos, Desempleados y Jubilados).

```{r}
# 1. Definición de variables
vars_agrupadas <- c("f_Edad", "f_Nivel.Educativo", "f_Situacion.Laboral")
nombres_vars <- c("Edad Agrupada", "Nivel Educativo Agrupado", "Situación Laboral Agrupada")

# 2. Bucle de Análisis
for (i in seq_along(vars_agrupadas)) {
  
  var_actual <- vars_agrupadas[i]
  nombre_actual <- nombres_vars[i]
  base_num <- 117 + (i - 1) * 3
  
  datos_temp <- datos
  
  # --- PROCESO DE AGRUPACIÓN ROBUSTO ---
  if (var_actual == "f_Edad") {
    datos_temp[[var_actual]] <- factor(ifelse(datos_temp[[var_actual]] %in% c('Menor de 18', '18 - 24'), 
                                       'Jóvenes', 'No Jóvenes'))
  } else if (var_actual == "f_Nivel.Educativo") {
    datos_temp[[var_actual]] <- factor(ifelse(datos_temp[[var_actual]] %in% c('ESO', 'FP medio / Bachillerato'), 
                                       'Nivel de Estudios Medios', 'Nivel de Estudios Superiores'))
  } else if (var_actual == "f_Situacion.Laboral") {
    # Usamos grepl para que detecte "Estudiante" aunque tenga espacios o variaciones
    datos_temp[[var_actual]] <- factor(ifelse(grepl("Estudiante", datos_temp[[var_actual]], ignore.case = TRUE), 
                                       'Estudiantes', 'No Estudiantes'))
  }
  
  datos_temp <- datos_temp %>% droplevels() %>% filter(!is.na(get(var_actual)))

  # --- DETECCIÓN DE ACEPTACIÓN 100% ---
  tabla_proporciones <- prop.table(table(datos_temp[[var_actual]], datos_temp$f_Intencion.Uso), 1)
  
  if ("Sí" %in% colnames(tabla_proporciones)) {
    exito_total <- rownames(tabla_proporciones)[which(tabla_proporciones[, "Sí"] == 1)]
    if (length(exito_total) > 0) {
      cat(paste0("\n**Nota:** El grupo '", paste(exito_total, collapse = ", "), 
                 "' tiene una aceptación del 100% en ", nombre_actual, ".\n"))
    }
  }

  cat(paste("\n### Análisis para:", nombre_actual, "\n"))
  
  # A. TABLA DE CONTINGENCIA (Se imprime siempre)
  t1 <- proc_freq(datos_temp, var_actual, 'f_Intencion.Uso',
            include.row_percent = TRUE, include.column_percent = FALSE) %>%
    compose(i=1, j=1, value = as_paragraph(nombre_actual), part = 'header') %>%
    set_caption(paste0("Tabla ", base_num + 1, ". Contingencia: ", nombre_actual, " vs Aceptación.")) %>%
    autofit()
  print(t1)
  ...
  n_niv_x <- length(unique(datos_temp[[var_actual]]))
  n_niv_y <- length(unique(datos_temp$f_Intencion.Uso))
  
  if (n_niv_x >= 2 & n_niv_y >= 2) {
    # B. Test Chi-Cuadrado
    res_chisq <- rstatix::chisq_test(x = datos_temp[[var_actual]], y = datos_temp$f_Intencion.Uso)
    t2 <- res_chisq %>% flextable() %>%
      set_caption(paste0("Tabla ", base_num + 2, ". Resultado Chi-cuadrado (", nombre_actual, ").")) %>%
      colformat_num(j = c("statistic", "p"), big.mark = ".", decimal.mark = ",") %>%
      bold(i = ~ p < 0.05, bold = TRUE) %>% autofit()
    print(t2)
    
    # C. Valores Esperados
    t3 <- rstatix::chisq_descriptives(res_chisq) %>%
      dplyr::rename(Variable = 'x', Aceptacion = 'y') %>% flextable() %>%
      set_caption(paste0("Tabla ", base_num + 3, ". Valores esperados: ", nombre_actual, ".")) %>%
      colformat_num(j = c("expected", "std.resid"), big.mark = ".", decimal.mark = ",") %>%
      autofit()
    print(t3)
  } else {
    cat("\n*No se realiza el test estadístico para esta variable por falta de diversidad en las respuestas (una categoría tiene 0 respuestas).*\n")
  }
  cat("\n---\n")
}
```

Una vez realizado este análisis, de las variables Edad y Situación Laboral tenemos unos valores “expected” > 5 por lo que el test es válido para estas 2 variables. Además ambas variables cuentan con un p-valor > 0,05 en los test de Chi-cuadrado, por lo tanto no existen relaciones entre la edad de los encuestados y el grado de aceptación ni entre la situación laboral de los encuestados y el grado de aceptación.
Por otro lado, hay valores esperados de Nivel Educativo que no son superiores a 5, por lo que realizamos un test de Fisher, para comprobar si existe una relación o no:

```{r}
# Creamos una nueva variable para no perder la original
datos$f_Nivel.Educativo_Agrupado <- factor(ifelse(
  datos$f_Nivel.Educativo %in% c('ESO', 'FP medio / Bachillerato'), 
  'Nivel de Estudios Medios', 
  'Nivel de Estudios Superiores'
))

tabla_cruzada <- table (datos$f_Nivel.Educativo_Agrupado, datos$f_Intencion.Uso)

res.fisher <- fisher_test(tabla_cruzada)

res.fisher %>%
  flextable() %>%
  set_caption(' Tabla 127. Resultados del Test de Fisher') %>%
  colformat_num(
    j = c("p"),
    big.mark = ".",
    decimal.mark = ","
...
Como el p-valor > 0,05, tampoco podemos rechazar la hipótesis nula y por tanto no existe una relación entre el nivel educativo máximo alcanzado por los encuestados y la aceptación de la barra de ensaladas.
